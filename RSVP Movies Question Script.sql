...
SELECT table_name, table_rows as Num_of_Rows
...
select
	sum(case when id is null then 1 else 0 end) as id_blanks,
	sum(case when title is null then 1 else 0 end) as title_blanks,
	sum(case when year is null then 1 else 0 end) as year_blanks,
	sum(case when date_published is null then 1 else 0 end) as date_published_blanks,
	sum(case when duration is null then 1 else 0 end) as duration_blanks,
    sum(case when country is null then 1 else 0 end) as country_blanks,
    sum(case when worlwide_gross_income is null then 1 else 0 end) as worlwide_gross_income_blanks,
    sum(case when languages is null then 1 else 0 end) as languages_blanks,
    sum(case when production_company is null then 1 else 0 end) as production_company_blanks
from movie;
...
select count(id) as No_of_movies_USA_and_India_in_2019
from movie
...
(
	select genre, 
    count(movie_id) as count_id 
    from genre group by genre
)

select * 
from tab_t 
where count_id = (select max(count_id) from tab_t);
...
with tab_g as
(
	select movie_id, count(genre) count_col 
    from genre 
    group by movie_id
)
...
SELECT title,
avg_rating,
dense_rank() OVER(ORDER BY avg_rating DESC) AS average_rating_rank

FROM ratings r, movie m where m.id = r.movie_id
...
where m.id = r.movie_id and avg_rating > 8 and  production_company is not null
...
dense_rank() OVER(ORDER BY Count(m.id) DESC)  genre_rank
from movie m
...
select name, count(r.movie_id) movie_count
from ratings r
...
dense_rank() OVER(ORDER BY sum(total_votes) DESC) prod_company_rank
from movie m
inner join ratings r
on m.id = r.movie_id
group by production_company
...
select name actress_name, count(m.id) movie_count,
...
select name actress_name, total_votes, count(m.id) movie_count, avg_rating actress_avg_rating, 
...
where r.avg_rating >8 and rm.category = 'actress' and genre = 'Drama'
